                                    ...
                                    <div class="table-responsive">
                                        <table id="datatable" class="display table table-striped table-hover dataTable">
                                            ...
                                            <tbody>
                                                ...
                                                $sql = "SELECT a.*,b.format_nomor_pendaftaran,DATE_FORMAT(a.created_at,'%d-%m-%Y') tgl FROM panduan a JOIN pansel b ON a.id_pansel=b.id WHERE a.deleted_at IS NULL $sql2 ORDER BY a.created_at DESC";
                                                ...
                                                foreach ($cn->result() as $rw) {
                                                    ...
                                                    echo '<td>' . $rw->judul. '</td>';
                                                    echo '<td><a href="'.base_url().'content/modal?action=viewdok&jenis=panduanicon&hid='.$rw->id.'" class="ls-modal">'. $rw->icon.'</a></td>';
                                                    echo '<td><a href="'.base_url().'content/modal?action=viewdok&jenis=panduanfile&hid='.$rw->id.'" class="ls-modal">'. $rw->file.'</a></td>';
        ...
        $("#datatable").on("click", ".del", function(e) {
            ...
            swal({
                ...
                if (willDelete) {
                    ...
                    $.ajax({
                        url: "<?php echo base_url(); ?>content/hapusPanduan",
